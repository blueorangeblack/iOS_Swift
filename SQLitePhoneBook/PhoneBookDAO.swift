...
class PhoneBookDAO {
    ...
    //정수 1개(번호), 문자열 3개(이름, 전화번호, 주소)로 구성
    typealias PhoneRecord = (Int, String, String, String)
    
    //SQLite 연결 및 초기화
    //lazy 지연생성 - 처음부터 만들지 않고 처음 사용할 때 생성
    ...
    func get(num:Int) -> PhoneRecord?{
        ...
        //throws가 있으면 예외처리해야 하고
        //let rs = try! self.fmdb.executeQuery(sql, values: [num])
        //throws가 없으면 예외처리하지 않아도 됨
